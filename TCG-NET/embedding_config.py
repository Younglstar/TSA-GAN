...
class EmbeddingConfig:
    ...
    # 4. 文件路径
    # 嵌入后的输出 parquet（静态 & 时序）
    EMBED_STATIC_OUTPUT_PATH: str = "embed_static.parquet"
    EMBED_TEMPORAL_OUTPUT_PATH: str = "embed_temporal.parquet"

    # 对应的 mask 输出
    EMBED_STATIC_MASK_OUTPUT_PATH: str = "embed_static_mask.parquet"
    EMBED_TEMPORAL_MASK_OUTPUT_PATH: str = "embed_temporal_mask.parquet"

    # 轻量级 meta 文件（保存这些路径），供 encdec 阶段读取
    EMBEDDINGS_FILE: str = "embeddings_meta.pkl"

    # 嵌入模型和编码器参数的保存位置（你原来应该已有）
