...
class EncoderDecoderConfig:
    ...
    BETA_KL_FINAL: float = 0.2  # <--- 修改：从一个更小、更合理的值开始
    # 因果关系损失项的权重
    GAMMA_CAUSAL: float = 1.0
    # 因果损失的目标值
    CAUSAL_TARGET_VALUE: float = 0.05  # <--- 新增：将硬编码的目标值移入配置
    # "Free Bits" 技术的阈值，为KL损失提供一个“豁免区”
    FREE_BITS_THRESHOLD: float = 0.05  # <--- 新增：引入Free Bits以防止KL完全消失
    ...
    KL_ANNEALING_CYCLE_EPOCHS: int = 100  # <--- 修改：适当延长周期，使增长更平缓
